# Read zipcode info from the text database.  Original README text included
# at the bottom of the file.
require 'date'

module Zip
  Info = Struct.new(:code, :city, :state, :lat, :lon, :tz)

  @@zip_cache = {}

  def get_zipinfo(code)
    if (info = @@zip_cache[code])
      info
    else
      line = `grep '^#{code},' db/zipdb.txt`
      if line.length > 2
        zip, city, state, lat, lon, tz = line.rstrip.split(',')
        info = Zip::Info.new(code, city, state, lat, lon, tz)
        @@zip_cache[zip] = info
      end
    end
  end
