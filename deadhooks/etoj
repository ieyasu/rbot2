...
if(stristr($_SERVER[SCRIPT_NAME], "jdict"))
    reply_exit("!jdict cannot be used directly.  Please use !etoj (English -> Japanese) or !jtoe (Japanese -> English)\n");
if(stristr($_SERVER[SCRIPT_NAME], "jtoe")) $nihongo = true;
	
if(!$args) {
    if($nihongo) {
        reply_exit("Shows the English translation of the inputted Japanese word.  Accepts romaji input only.  Usage: !jtoe <word>");
    }
    else {
        reply_exit("Shows the Japanese translation of the inputted English word.  Usage: !etoj <word>");
    }
}
...
if(!$fp)
    reply_exit("Could not access freedict.");
