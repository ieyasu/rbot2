...
require('inc/ircdb.inc');
	...
	if(trim($args) == "gimmetengrand") {
		mysql_query("delete from roulette where id = '$source'");
		mysql_query("insert into roulette values('$source', '10000')");
		print "ok, you have 10000";
		exit();
	}
	...
	$a = mqns("select * from roulette where id = '$nick'");
	if($a) {
		$money = $a[0][money];
	} else {
		$money = 1;
	}
	if($betamount > $money) die('You only have $' . $money);
	if($betamount != intval($betamount)) die("can't parse bet amount $betamount");
	if($betamount < 0) die("Can't parse bet amount $betamount");
	...
	if($betamount != 0) die("Can't parse bet $bet");
}
	...
	if($betamount != 0) {
		...
		if(in_array($val, $wins)) {
			...
			$money -= $betamount;
			if($money == 0) $money = 1;
			print 'you lose $' . $betamount;
			print ', you have $' . $money;
		}
		mysql_query("delete from roulette where id = '$nick'");
		mysql_query("insert into roulette values('$nick', '$money')");
	} else {
		print "$list[$val], you have " . '$' . $money;
	}
	//if($betamount != 0) mysql_query("delete from roulette where id = '$nick'");
	mysql_query("delete from roulette where money <= 1");
?>
