<?
  ...
  elseif($cmd == "stats") {
    if(!$arg[1])
      die("Usage: !ro [stats] <monster>.  Flags are Aggressive, Loots, Detects, aSsists, Boss, Miniboss, spasH damage, Immobile, Passive, Ranged attack");
    $query = trim($arg[1]." ".$arg[2]);
  }
  else {
    $query = $args;
...
// Added 01/25/04 dkb
// This just goes by file order now (02/23/04 dkb)
	$found = 0;
	$lines = explode("\n", $src);
	$currmatch = array();
	foreach($lines as $line) {
		...
		$from = strtolower(trim($query));
		$to = strtolower(trim($row[0]));
		$from = preg_replace("/[^a-z\?]/", "", $from);
		$to = preg_replace("/[^a-z\?]/", "", $to);
		$to = substr($to, 0, strlen($from));
		if($from == $to) {
			$currmatch = $row;
			$found = 1;
			break;
